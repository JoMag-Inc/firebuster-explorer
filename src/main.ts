...
const PRESET_LOCATIONS: readonly LocationPreset[] = [
...
function normalizeRow(candidate: unknown): TTFResult | null {
...
function pickRows(payload: unknown): TTFResult[] {
  ...
  while (queue.length > 0) {
    ...
    if (Array.isArray(current)) {
      const rows = current.map((item) => normalizeRow(item)).filter((item): item is TTFResult => item !== null)
...
async function fetchTTF(latitude: number, longitude: number): Promise<TTFResult[]> {
  if (!token) {
...
function clearResponse(): void {
  ...
  if (summary) summary.textContent = 'No request made yet.'
  if (tableBody) tableBody.innerHTML = ''
  if (raw) raw.textContent = ''
}

function renderResponse(latitude: number, longitude: number, rows: TTFResult[]): void {
  const summary = document.getElementById('response-summary')
  const tableBody = document.getElementById('response-rows')
  const raw = document.getElementById('response-raw')

  if (!summary || !tableBody || !raw) return
...
function parseCoordinate(id: 'latitude' | 'longitude'): number {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const presetButtons = document.querySelectorAll<HTMLButtonElement>('[data-lat][data-lon]')
